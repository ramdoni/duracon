...
class Pabrikjadwaltulangan extends CI_Controller {
	...
	function __construct()
	{
		...
		$this->load->model('Tulanganschedule_model');
		$this->load->model('Tulanganscheduleplan_model');

		$this->model = $this->Tulanganschedule_model;
	}
	...
	public function index()
	{
		...
		$params['page'] = 'pabrikjadwaltulangan/index';
	...
	public function insert()
	{
		if($this->input->post())
		{
			...
			if($this->db->insert($this->model->t_table, $param)):
				...
				$product_table = $this->input->post('tulangan_id');
				foreach($product_table as $key => $i)
				{
					$param = [];
					$param['tulangan_schedule_id'] = $insert_id;
					$param['tulangan_id'] = $i;
					...
					$this->db->insert('tulangan_schedule_plan', $param);
				}
			endif;

			$this->session->set_flashdata('messages', 'Data berhasil disimpan');

			redirect('pabrikjadwaltulangan/index','location');
		}
		
		$params['page'] = 'pabrikjadwaltulangan/form';
	...
	public function edit($id=0)
	{
		...
		if($this->input->post())
		{
			...
			if($this->db->update($this->model->t_table, $param)):
				...
				$product_table = $this->input->post('tulangan_id_old');
				foreach($product_table as $key => $i)
				{
					$param = [];
					$param['tulangan_schedule_id'] = $id;
					$param['tulangan_id'] = $i;
					...
					$pln = $this->db->get_where('tulangan_schedule_plan', ['id' => $post['plan_id'][$key]])->row_array();
					...
					$this->db->update('tulangan_schedule_plan', $param);
					$this->db->flush_cache();
				}
		...
		$list_plan = $this->Tulanganscheduleplan_model->get_by_productschedule($model['id']);
		$list_plan_revisi = $this->Tulanganscheduleplan_model->get_by_productschedule($model['id'],1);
	...
	public function delete($id=0)
	{
		...
		redirect(site_url('pabrikjadwaltulangan'));
	}
	...
	public function print_jadwal($id=0)
	{
		...
		$params['list_plan'] = $this->Tulanganscheduleplan_model->get_by_productschedule($id);
		$params['list_plan_revisi'] = $this->Tulanganscheduleplan_model->get_by_productschedule($id,1);

		
		$html = $this->load->view('pages/pabrikjadwaltulangan/print', $params, true);
	...
	public function reportmingguan($id)
	{
		...
		$params['page'] = 'pabrikjadwaltulangan/reportmingguan';
		$params['data'] = $model;
		$params['list_plan'] = $list_plan;
		$params['list_plan_revisi'] = $list_plan_revisi;
	...
	public function lembarkerja($id)
	{
		...
		$params['page'] = 'pabrikjadwaltulangan/lembarkerja';
		$params['data'] = $model;
		$params['list_plan'] = $list_plan;
		$params['list_plan_revisi']	= $list_plan_revisi;
