 ...
 **/
 ...
 class Tulanganscheduleplan_model extends CI_Model
 {
	var $t_table 	= 'tulangan_schedule_plan';
    ...
    {
    	$this->db->from($this->t_table);
    	$this->db->select('tulangan_schedule_plan.*, tulangan.kode as product, tulangan.weight');
    	$this->db->join('tulangan', 'tulangan.id=tulangan_schedule_plan.tulangan_id');

    	if($revisi == 1)
    	{
    		$this->db->where([$this->t_table.'.tulangan_schedule_id' => $id, 'is_revisi' => 1]);
    	}else
    		$this->db->where([$this->t_table.'.tulangan_schedule_id' => $id,'is_revisi' => 0]);
