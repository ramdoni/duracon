                ...
                <tr class="even pointer">
                    ...
                    <td>
                      <?php if($item['position'] == 2):?>
                         <a href="<?=site_url("marketingsalesorder/proccess/{$item['id']}")?>" title="Proccess"><span class="btn btn-success btn-xs"> Proses <i class="fa fa-arrow-right"></i></span></a>
                      <?php endif;?>
                      <span onclick="detail_sales_order(<?=$item['quotation_order_id']?>,<?=$item['id']?>,'<?=$item['no_po']?>')" target="_blank" class="btn btn-default btn-xs" title="Detail Quotation"><i class="fa fa-search-plus"></i> Detail</span>
                       ...
                       <?php if($item['position'] >= 5 and $item['sales'] == ""):?>

                        <?php if($this->db->get_where('surat_izin_kirim', ['sales_order_id' => $item['id']])->num_rows() >= 1){ ?>
                          <a href="<?=site_url('marketingsalesorder/viewsik/'. $item['id'])?>" target="_blank" class="btn btn-default btn-xs" title="Buat Surat Izin Kirim"><i class="fa fa-search-plus"></i> Lihat SIK <br />(Surat Izin Kirim)</a>
                          <?php } else{ ?>
                          <a href="<?=site_url('marketingsalesorder/createsik/'. $item['id'])?>" class="btn btn-default btn-xs" title="Buat Surat Izin Kirim"><i class="fa fa-plus"></i> Buat SIK <br />(Surat Izin Kirim)</a>
                          <?php } ?>
